...
const Flow = Vex.Flow;
const Element = Flow.Element;
const Fraction = Flow.Fraction;
const Tuplet = Flow.Tuplet;
const Stem = Flow.Stem;
...
export class Beam extends Element {
  ...
  handleCrossStaffBeam() {
    ...
    for (const note of this.notes) {
      if (!stemDirection) {
        ...
        this.slope = 0;
        this.y_shift = (lastNote.getStemExtents().topY - firstNote.getStemExtents().topY) / 2
                        + 2.5 * -firstNote.getStemDirection();
        return true;
      }
    }
  ...
  calculateSlope() {
    ...
    const {
      ...
      render_options: { max_slope, min_slope, ideal_slope_ratio, slope_iterations, slope_cost },
    } = this;
    ...
    const idealSlope = initialSlope * ideal_slope_ratio;
  ...
  applyStemExtensions() {
    const {
      notes, slope, y_shift, beam_count,
      render_options: {
        show_stemlets,
        flat_beam_offset,
        flat_beams,
    ...
    for (let i = 0; i < notes.length; ++i) {
      ...
      const beamExtension = note.getStemDirection() === Stem.UP
